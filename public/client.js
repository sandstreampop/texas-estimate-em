...
function ListTodos({ items =[]  }) {
    const [deletedIdxs, setDeletedIdxs] = export_default1.useState([]);
    return export_default1.createElement("div", null, export_default1.createElement("ul", {
        className: "list-group"
    }, items.map((todo, index)=>{
        const deleted = deletedIdxs.indexOf(index) !== -1;
        return export_default1.createElement("li", {
            key: index,
            className: "list-group-item",
            style: {
                color: deleted ? "red" : "green"
            }
        }, todo.task, export_default1.createElement("button", {
            type: "button",
            className: "ml-2 mb-1 close",
            "aria-label": "Close",
            onClick: ()=>setDeletedIdxs([
                    ...deletedIdxs,
                    index
                ])
...
const { todos  } = window.__INITIAL_STATE__ || {
    todos: []
};
export_default3.hydrate(export_default1.createElement(App, {
    todos: todos
